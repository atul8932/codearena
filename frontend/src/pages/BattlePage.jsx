...
function TimerDisplay({ seconds }) {
  ...
  const glowClass = isCritical ? 'text-glow-red' : isUrgent ? 'text-glow-gold' : 'text-glow-green';
  return (
    <div className={`font-mono text-base font-bold tabular-nums ${isCritical ? 'animate-pulse' : ''} ${glowClass}`}
      style={{ color: isCritical ? 'var(--red)' : isUrgent ? 'var(--bronze)' : 'var(--accent)' }}>
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-3 sm:px-4 shrink-0"
        ...
        <span className="text-sm font-bold shrink-0 text-glow-subtle" style={{ color: 'var(--text)' }}>
        ...
        <span className="text-xs sm:text-sm font-medium truncate text-glow-subtle" style={{ color: 'var(--text-muted)' }}>{problem?.title || 'Battle'}</span>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <span className="text-xs font-mono hidden sm:block text-glow-subtle" style={{ color: 'var(--text-dim)' }}>{roomId}</span>
          {canSwitch && (
